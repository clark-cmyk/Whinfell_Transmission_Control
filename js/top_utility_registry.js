...
(function topUtilityRegistry(global) {
  ...
  function shortcutHint(entry) {
    if (!entry.shortcut) return '';
    const mod = entry.shortcutMod || 'alt-shift';
    const SS = global.WTM_ShellShortcuts;
    if (SS?.formatModLabel) {
      const modLabel = SS.formatModLabel(mod);
      if (SS.IS_MAC) return `${modLabel}${entry.shortcut.toUpperCase()}`;
      return `${modLabel}+${entry.shortcut.toUpperCase()}`;
    }
    const fallback = mod.split('-').map((p) => p.charAt(0).toUpperCase() + p.slice(1)).join('+');
    return `${fallback}+${entry.shortcut.toUpperCase()}`;
  }

  function resolveUtilityTitle(entry) {
    let base = '';
    if (entry.title) base = entry.title;
    else if (HEADER_LAYOUT.compactMeta && entry.compactLabel && entry.label) {
      base = `${entry.compactLabel} — ${entry.label}`;
    } else {
      base = entry.compactLabel || entry.label || '';
    }
    const hint = shortcutHint(entry);
    if (hint && base && !base.includes(hint)) return `${base} (${hint})`;
    if (hint && !base) return hint;
    return base;
  }
  ...
  function createUtilityNode(entry) {
    ...
    if (entry.shortcut) {
      node.dataset.shortcut = entry.shortcut;
      node.dataset.shortcutMod = entry.shortcutMod || 'alt-shift';
    }
    return node;
  }
