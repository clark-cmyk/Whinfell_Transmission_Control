...
(function deskDataOps(global) {
  ...
  const BUILD = '1.0-DESK-OPS-2026-07-07';
  ...
  async function refreshBasisWatch() {
    ...
    const getState = () => global.appState || {};
    const hooks = {
    ...
    try {
      if (typeof global.WTM_BasisWatch.reloadCurve === 'function') {
        await global.WTM_BasisWatch.reloadCurve(getState(), hooks);
        return true;
      }
      ...
      if (typeof global.WTM_BasisWatch.refresh === 'function') {
        await global.WTM_BasisWatch.refresh(getState(), hooks);
        return true;
      }
    } catch (err) {
