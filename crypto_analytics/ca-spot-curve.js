/**
 * CME Crypto Analytics — Implied Rate Spot Curve (Chunk 29 LWC).
...
(function cryptoAnalyticsSpotCurve(global) {
  ...
  function ensureMount(wrap) {
    ...
    if (!mount) {
      const old = wrap.querySelector('canvas.ca-chart-canvas, .ca-chart-canvas');
      if (old && old.tagName === 'CANVAS') old.remove();
      mount = document.createElement('div');
      mount.className = 'wtm-lwc-mount ca-chart-canvas';
      mount.setAttribute('role', 'img');
      mount.setAttribute('aria-label', 'Spot curve chart');
  ...
  function createSpotCurve(mountEl, callbacks) {
    ...
    function render() {
      ...
      if (title) title.textContent = `${callbacks.getAsset()} Implied Spot Curve`;
